...
Labels use English categories and recognizable lowercase tool actions, with call counts (not distinct-file counts). The `@tintinweb/pi-subagents` tools `Agent`, `SubagentWorkflow`, `get_subagent_result`, and `steer_subagent`, plus the official `pi-subagents` tools `subagent` and `subagent_supervisor`, count under `Subagent`. Their rows use bounded action/target labels and never include the large prompt, workflow script, task, or steering message. Official `subagent`/`subagent_supervisor` rows keep their tool name in front of the action, so a bare management verb stays attributable (for example `subagent status <run id>`, `subagent_supervisor reply <request id>`, or `subagent scout` for a launch). Official workflow calls label themselves from their own short fields — `workflowScriptPath`, workflow resource name, `lane.key`, or `mission.title`/`summary` — and append the declared `preflight.lanes` count (for example `subagent workflow parallel review · 3 lanes`) without parsing the script. Known tools are categorized by registered name, never by substring or shell-command guesses; lookalikes and other custom/MCP names remain visible under `Other`. Unknown and MCP proxy rows label themselves from their own short fields such as `tool`, `server`, `search`, or `action` (for example `mcp jenkins_get_running_builds`) instead of showing a bare tool name. Status glyphs (`○`, `…`, `✓`, `✗`) remain meaningful without color; colors come from Pi's current theme.

Headers use `Process` for completed groups, `Working` for active groups, `Unfinished` for stopped unresolved groups, and `Failed` for failed groups. Builtin `bash`, `powershell`, and `Other` activity (including custom/MCP tools) stays entirely in the one-line header while folded: no transient `Running:` or `Unfinished:` line exposes their command, query, or target. Known non-shell tools retain one bounded activity line with their target/path; after the run stops, outstanding known non-shell calls use `Unfinished:` instead. Failed groups keep the compact `Failed` header in warning color while only the diagnostic line and failed expanded rows use error red. The diagnostic shows a failure count and a short reason followed by the failed action/name and bounded target (for example `Failed: exit code 1 · bash npm test`). A parallel current/outstanding call shares that same status line. The reason comes first so long targets cannot hide it; pending/unresolved calls are never labeled completed. These compact hints do not replace saved details. Only successful `edit` results with a valid saved, numbered Pi diff get `+added −removed` statistics.

Groups expand independently, with Pi's standard outer message spacing. Click the group header or its activity line to show items, then an item row to inspect output and edit diffs inline. An opened item shows Pi's native card first; the exact saved-arguments inspector stays collapsed behind its own `Arguments` row and is only serialized when opened. Open tool/native detail components are retained across streamed results; nested controls provided by a registered native renderer keep their own state. The plugin does not infer arbitrary Agent/MCP child executions from output text or invent a universal nested-call schema. Later tool calls never pull earlier assistant text back into a fold. Plain-text answers without tools or thinking create no empty process row. This is **not** a final-answer-only Focus view.
...
Both common subagent notification protocols are supported by exact type. Adjacent displayed `@tintinweb/pi-subagents` cards with `customType: "subagent-notification"` get a compact group such as `Process · Subagents 3 completed`; click the group and then a task row to reveal bounded structured metadata. Adjacent displayed official `pi-subagents` cards with `customType: "subagent-notify"` get `Process · Subagent notifications N`. Its exact `subagent_supervisor_request` messages also fold: progress reports use `Process · Supervisor N updates`, while groups containing a decision or interview request stay visibly marked `Attention · Supervisor …`. Exact `subagent_control_notice` cards also fold: `event.reason: "supervisor_request"` joins the Supervisor group, while other watchdog/control reasons such as `tool_open_threshold` use `Attention · Subagent N alerts`. Exact `subagent_supervisor_reply` entries fold there too. The remaining automatic lifecycle cards fold by exact type as well: `subagent_steering_notice` (`Subagent steering`), `subagent_watchdog_warning` (`Watchdog`, with `Blocked` for blockers), `subagent-wait-subscription` (`Subagent waits`), `subagent-slash-result` (`Subagent commands`), and `subagent-incremental-child-notify` (`Workflow child alerts`, which the official package only displays for non-completed workflow children and sends without structured details). User-requested output such as `subagent-slash-text-result` and `subagents-admin` stays native, and internal `subagents:record` entries are never touched. When a reply in the same group is newer than the matching run's queued alert, that resolved alert no longer keeps the header in `Attention`; its native history remains available on expansion. Attention headers use the theme's warning color, not its error color; they indicate required attention, not execution failure. Opening either official group reuses that package's own expanded native renderer, and folding never changes request IDs, reply state, or supervisor/subagent communication. Any direct chat child — a folded subagent card, an extension notice, or slash-command output such as `/session` — seals the preceding display fragment, so later assistant text renders after it instead of leaving that output pinned at the bottom. The folded view never parses either card's text/XML, reads transcript output files, or associates `subagents:record` entries. Lookalike types and all unrelated custom components split the run and remain native. Protocol compatibility is checked against `@tintinweb/pi-subagents` 0.19.0 and `pi-subagents` 0.66.0–0.67.0; future upstream protocol changes may require an adapter update.

Failures, aborted responses, truncation and unfinished calls are indicated outside the collapsed details. Provider warnings such as `Error: Request timed out.` render where they occurred, so a retry's output appears below them instead of leaving the warning pinned under later text. Pi's extension dialogs, notices, editor and execution behavior remain native.
...
- Custom extension notices and user-run `!` shell commands stay native and may occupy multiple lines. Routine `MCP: direct tools refreshed (+N, ~N, -N)` info toasts are suppressed while folding is enabled because they duplicate the Process count; MCP errors, authentication warnings, and all other notices remain native. The only custom-message exceptions are the exact displayed `subagent-notification`, `subagent-notify`, `subagent_supervisor_request`, `subagent_control_notice`, and exact `subagent_supervisor_reply` entries described above; unknown custom messages and other custom entries remain visible.
