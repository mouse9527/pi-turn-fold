...
- Custom extension notices and user-run `!` shell commands stay native and may occupy multiple lines. The only custom-message exceptions are the exact displayed `subagent-notification`, `subagent-notify`, `subagent_supervisor_request`, and supervisor-request-specific `subagent_control_notice` cards described above; unknown custom messages, other control notices, and all custom entries remain visible.
