...
The canonical chat remains flat and in native order. The adapter projects user messages, lightweight turn anchors, and native notices into a second `Container`. Hidden native tools and assistants are absent from that visible tree, not rendered and then erased. Adjacent projected `CustomMessageComponent` instances are replaced only when their private message is displayed and has exact `customType: "subagent-notification"` or `customType: "subagent-notify"`; protocol changes split groups and canonical components remain untouched. No rendered text/XML matching or generic custom-component folding is used.
...
Native custom entries occasionally insert directly into `chatContainer.children`. The adapter reconciles that rare insertion path explicitly, preserving notification-group chronology and using every unrelated visible component as an adjacency boundary; ordinary streaming events append without rescanning the canonical tree.
...
Each group keeps fixed `Read / Search / Run / Edit / Subagent / Other` call counters and pending/running/failure maps keyed by tool ID. Only exact `Agent`, `SubagentWorkflow`, `get_subagent_result`, `steer_subagent`, and official `subagent` names enter `Subagent`; lookalikes stay `Other`. Their compact actions/targets come from bounded identifiers and descriptions, never prompt/workflow-script/task/message payloads. Saved expanded arguments and results are unchanged. Repeated results first remove previous status contributions, so out-of-order completions and corrected results do not double count. The oldest still-running non-shell call supplies `Running:` while the turn is running; after the turn stops, that outstanding call is labeled `Unfinished:`, without changing its execution status or inventing a result. Builtin `bash` and `powershell` activity stays entirely in the one-line header while folded, with no transient second-line hint; other tools retain their bounded target/path. Expanded item rows and saved argument/output inspectors are unchanged. Failures show the brief reason first, then the failed action/original custom name and bounded target; a parallel current/outstanding call follows on the same single status line. Unresolved calls remain unfinished even after the agent ends. Names, not shell-command heuristics, determine categories; unknown names stay intact in the bounded item label.
...
## Subagent notification cards

Notification headers count actual cards, independently from `Subagent` tool-call counts. For `@tintinweb/pi-subagents`, status and row descriptions come only from structured `subagent-notification` details; opening a task reads bounded `resultPreview` and `outputFile` prefixes plus numeric tool-use/token/cost/duration fields. For official `pi-subagents`, `subagent-notify` messages do not persist equivalent structured details, so the closed group counts notification cards and opening it creates temporary expanded `CustomMessageComponent` clones using the official package's own renderer. Canonical cards are never mutated, and this extension never copies the official package's content parser.

Closed groups retain no row/detail/native-clone components. Projection reconciliation reuses surviving groups where possible and disposes stale rows/groups after removals, clears, reloads, or direct custom-entry splices. `/fold off` renders the canonical native cards immediately and closes projected notification details; `/fold on` returns to the current folded projection without cloning session entries. The adapter never reads output files, searches transcript records, interprets `subagents:record`, or parses arbitrary custom-message content.
