...
Notification headers count actual cards, independently from `Subagent` tool-call counts. Status and row descriptions come only from structured message details. Closed groups retain no row/detail components; opening a group builds lightweight rows, and opening one row reads at most bounded `resultPreview` and `outputFile` prefixes plus numeric tool-use/token/cost/duration fields. All compact text is terminal-control sanitized and cell-width clipped. The adapter never reads output files, searches transcript records, interprets `subagents:record`, or formats arbitrary custom-message content.

Projection reconciliation reuses surviving groups where possible and disposes stale rows/groups after removals, clears, reloads, or direct custom-entry splices. `/fold off` renders the canonical native cards immediately and closes synthetic notification details; `/fold on` returns to the current folded projection without cloning session entries.
